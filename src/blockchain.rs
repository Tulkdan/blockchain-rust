use super::{Block, block, Hashable, Hash};
...
impl Blockchain {
    ...
    pub fn update_with_block(&mut self, block: Block) -> Result<(), BlockValidationError> {
        ...
        if block.index != i as u32 {
            ...
            // Check if is not a genesis block
        ...
        if let Some((coinbase, transactions)) = block.transactions.split_first() {
            ...
            for transaction in transactions {
                ...
                if output_value > input_value {
                    return Err(BlockValidationError::InsufficiendInputValue);
                }
            ...
            self.unspent_outputs.retain(|output| !block_created.contains(output));
